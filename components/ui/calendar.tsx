...
function Calendar({
  ...
  const handleDayClick = (day: Date) => {
    const formattedDatePlusOne = new Date(day)
    formattedDatePlusOne.setDate(formattedDatePlusOne.getDate() + 1)
    const formattedDatePlusOneString = formattedDatePlusOne
      .toISOString()
      .split("T")[0]

    router.push(`/donate/date/${formattedDatePlusOneString}`)
  }
    ...
    <DayPicker
      ...
      classNames={{
        ...
        day: cn(
          ...
          "h-8 w-8 p-0 font-normal aria-selected:opacity-100 hover:text-primary"
        ),
        day_range_start: "day-range-start",
        day_range_end: "day-range-end",
        day_selected:
          "bg-zinc-900/80 outline-primary/20 outline outline-2 text-primary hover:opacity-80 focus:text-primary text-primary hover:text-rose-500",
        day_today: "outline-primary/10 outline text-primary",
        day_outside:
